...
public class Group_Calendar_UI extends Frame{
	...
	public Group_Calendar_UI(Session session, String gname){
		this.gname = gname;
		...
		//JScrollPane[][] sp = new JScrollPane[6][7];

		//scheduleList
		//[s.getUserId] s.getTitle() 형식으로 textArea에 띄우고
		//해당 날짜의 내용을 date_textArea에 넣는다.
		//Array리스트 사이즈 만큼 포문을 돌린다.
		//Array리스트에서 해당하는 일을 찾아 textArea에 넣는다.
		//기존값이 있는 경우 누적 적용한다.
		//DateList의 길이 만큼 for문을 돌리고
		//
		...
		for(int i = 0; i < date_textArea.length; i++){
			for(int j = 0; j < date_textArea[i].length; j++){
				...
				date_textArea[i][j].setBackground(new Color(255,255,255));
	...
	public void CalendarButtonUpdate(Calendar calendar, int monthValue){
		...
		for(int i = 0; i < dateList.size(); i++){			
			if(!dateList.equals("")){
				String[] tempScheduleListDate_str = tempScheduleList.get(i).getDate().split(" ");
				if(dateList.get(i).equals(tempScheduleListDate_str[2])){
					String str = "["+ tempScheduleList.get(i).getTitle() + "] " + tempScheduleList.get(i).getTitle(); 
		...
		for(int i = 0; i < date_textArea.length; i++){			
			for(int j = 0; j < date_textArea[i].length; j++){
				int count = 0;
				date_textArea[i][j] = new JTextArea(scheduleList.get(count));
				count++;
			}
		}
	}
